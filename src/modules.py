...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(outputs.shape[0]):
            for j in range(int(int(outputs.shape[1]) / 2)):
                sum = 0
                for k in range(int(int(outputs.shape[2]) / 2), outputs.shape[2]):
                    sum += outputs[i][j][k]
                sum = tf.cast(sum, tf.float32)
                sum *= 2
                sum /= int(outputs.shape[2])
                for k in range(int(int(outputs.shape[2]) / 2), outputs.shape[2]):
                    loss += abs(sum - outputs[i][j][k])
    ...
    return outputs, print_op_list, loss
...
def multihead_attention_mask(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Multiplication
        #print('outputs before mult ', outputs.shape)
        outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1])) # (h*N, T_q, T_k)
        #print('outputs before scale ', outputs.shape)
        # Scale
        outputs = outputs / (K_.get_shape().as_list()[-1] ** 0.5)
        ...
        # print(outputs)
        #print('outputs before usemyfunc ', outputs.shape)
        # print('zlp', int(outputs.shape[0]))
        #chk = tf.reshape(-1, outputs.shape[2])
        key_masks = tf.tile(key_masks, [num_heads, 1])  # (h*N, T_k)
        key_masks = tf.tile(tf.expand_dims(key_masks, 1), [1, tf.shape(queries)[1], 1])  # (h*N, T_q, T_k)
        #print(key_masks)
        if usemyfunc:
            tpaddings = tf.zeros_like(outputs)
            toutputs = tf.where(tf.equal(key_masks, 0), tpaddings, outputs)
            toutputs = toutputs[:, :int(height / 2), int(int(height) / 2): height]
            attn = toutputs
            newkey_masks = key_masks[:, :int(height / 2), int(int(height) / 2): height]
            tsum = tf.reduce_sum(toutputs, 2, keepdims=True)
            cast_toutputs = tf.cast(toutputs != 0, tf.float32)
            if len(cast_toutputs.shape)>=2:
                # 部分被mask
                num = tf.reduce_sum(cast_toutputs, 2, keepdims=True)
            else:
                # 全被mask
                tones = tf.cast(tf.ones_like(toutputs), tf.float32)
                num = tf.reduce_sum(tones, 2, keepdims=True)
            tsum = tf.div(tsum, num)
            tsum = tf.tile(tsum, [1, 1, int(height) - int(int(height) / 2)])
            toutputs = tf.where(tf.equal(newkey_masks, 0), tsum, toutputs)
            tsum = tf.cast(tf.subtract(tsum, toutputs), tf.float32)
            tsum = tf.square(tsum)
            loss = tf.reduce_sum(tsum)
            loss = loss / (tsum.get_shape().as_list()[-1])

        #print('outputs before key mask ', outputs.shape)
        # Key Masking
        ...
        paddings = tf.ones_like(outputs)*(-2**32+1)
        outputs = tf.where(tf.equal(key_masks, 0), paddings, outputs) # (h*N, T_q, T_k)
        #print('outputs before causality ', outputs.shape)
        ...
        #print('outputs before cloze ', outputs.shape)
        # Cloze = Current blinding
        if cloze:
            range_q_vals = tf.expand_dims(tf.range(0, tf.shape(outputs)[1]), 1)
            range_k_vals = tf.expand_dims(tf.range(0, tf.shape(outputs)[2]), 0)
        ...
        #print('outputs before activation ', outputs.shape)
        # Activation
        outputs = tf.nn.softmax(outputs) # (h*N, T_q, T_k)

        #print('outputs before q mask ', outputs.shape)
        # Query Masking
        query_masks = tf.tile(query_masks, [num_heads, 1]) # (h*N, T_q)
        query_masks = tf.tile(tf.expand_dims(query_masks, -1), [1, 1, tf.shape(keys)[1]]) # (h*N, T_q, T_k)
        outputs *= query_masks # broadcasting. (N, T_q, C)

        #print('outputs before drop out ', outputs.shape)
        # Dropouts
        outputs = tf.layers.dropout(outputs, rate=dropout_rate, training=tf.convert_to_tensor(is_training))
        #print('outputs before sum ', outputs.shape)
        # Weighted sum
        outputs = tf.matmul(outputs, V_) # ( h*N, T_q, C/h)
        #print('outputs before restore shape ', outputs.shape)
        # Restore shape
        outputs = tf.concat(tf.split(outputs, num_heads, axis=0), axis=2 ) # (N, T_q, C)
        #print('outputs before conn ', outputs.shape)
        # Residual connection
        outputs += queries
        #print('outputs before normal ', outputs.shape)
        # Normalize
        outputs = normalize(outputs) # (N, T_q, C)
        #print('outputs after all ', outputs.shape)
